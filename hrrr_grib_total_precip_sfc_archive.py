...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download HRRR total precipitation surface GRIB files and save them in the raw archive layout."
    )
    parser.add_argument(
        "--date",
...
def load_or_download_grib(
    ...
    raw_file = run_dir / f"hrrr.t{cycle_hour:02d}z.wrfsfcf{forecast_hour:02d}.apcp_surface.grib2"
...
def archive_gribs(date_str: str, max_forecast_hour: int, timeout: int, output_dir: Path, compress_output: bool) -> int:
    raw_dir = output_dir / date_str / "raw_grib_total_precip"
    latest_cycle = detect_latest_cycle(date_str)
    session = requests.Session()
    session.headers.update({"User-Agent": "hrrr-grib-total-precip-archive/1.0"})

    saved_files = 0
    print(f"Starting HRRR total precip GRIB archive run for {date_str}", flush=True)
    ...
    if saved_files == 0:
        raise SystemExit("No HRRR total precip GRIB files were available for the requested UTC date.")
