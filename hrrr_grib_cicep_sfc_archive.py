...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download HRRR surface categorical ice pellets GRIB files and save them in the raw archive layout."
    )
    parser.add_argument(
        "--date",
...
def load_or_download_grib(
    ...
    raw_file = run_dir / f"hrrr.t{cycle_hour:02d}z.wrfsfcf{forecast_hour:02d}.cicep_surface.grib2"
...
def archive_gribs(date_str: str, max_forecast_hour: int, timeout: int, output_dir: Path, compress_output: bool) -> int:
    raw_dir = output_dir / date_str / "raw_grib_cicep"
    latest_cycle = detect_latest_cycle(date_str)
    session = requests.Session()
    session.headers.update({"User-Agent": "hrrr-grib-cicep-archive/1.0"})

    saved_files = 0
    print(f"Starting HRRR surface categorical ice pellets GRIB archive run for {date_str}", flush=True)
    ...
    if saved_files == 0:
        raise SystemExit("No HRRR surface categorical ice pellets GRIB files were available for the requested UTC date.")
