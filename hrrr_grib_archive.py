...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download HRRR 2 m temperature GRIB files and save them in the raw archive layout."
    )
    parser.add_argument(
        "--date",
...
def archive_gribs(date_str: str, max_forecast_hour: int, timeout: int, output_dir: Path) -> int:
    raw_dir = output_dir / date_str / "raw_grib"
    latest_cycle = detect_latest_cycle(date_str)
    session = requests.Session()
    session.headers.update({"User-Agent": "hrrr-grib-archive/1.0"})

    saved_files = 0
    print(f"Starting HRRR GRIB archive run for {date_str}", flush=True)
    ...
    if saved_files == 0:
        raise SystemExit("No HRRR GRIB files were available for the requested UTC date.")
