...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download a combined HRRR surface GRIB archive with the currently tracked variables."
    )
    parser.add_argument(
        "--date",
...
def load_or_download_grib(
    ...
    raw_file = run_dir / f"hrrr.t{cycle_hour:02d}z.wrfsfcf{forecast_hour:02d}.full_surface.grib2"
    cached_files = [
        raw_file,
...
def archive_gribs(date_str: str, max_forecast_hour: int, timeout: int, output_dir: Path, compression: str) -> int:
    raw_dir = output_dir / date_str / "raw_grib_full_surface"
    latest_cycle = detect_latest_cycle(date_str)
    session = requests.Session()
    session.headers.update({"User-Agent": "hrrr-grib-full-surface-archive/1.0"})

    saved_files = 0
    print(f"Starting combined HRRR surface GRIB archive run for {date_str}", flush=True)
    ...
    if saved_files == 0:
        raise SystemExit("No combined HRRR surface GRIB files were available for the requested UTC date.")
