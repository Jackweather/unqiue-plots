...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download an HRRR entire-atmosphere GRIB archive for the tracked severe-weather fields."
    )
    parser.add_argument(
        "--date",
...
def archive_gribs(date_str: str, max_forecast_hour: int, timeout: int, output_dir: Path, compress_output: bool) -> int:
    ...
    session = requests.Session()
    session.headers.update({"User-Agent": "hrrr-grib-entire-atmosphere-archive/1.0"})

    saved_files = 0
    print(f"Starting HRRR entire-atmosphere GRIB archive run for {date_str}", flush=True)
    ...
    if saved_files == 0:
        raise SystemExit("No HRRR entire-atmosphere GRIB files were available for the requested UTC date.")
